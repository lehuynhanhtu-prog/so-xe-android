...
Mã khởi chạy nằm trong `windows-portable/`. Quy trình GitHub Actions biên dịch bộ khởi chạy gọn nhẹ và tạo file `So-Xe-Windows-Portable-1.1.10.zip`. Giải nén rồi chạy; bản 1.1.10 tự đóng máy chủ Sổ Xe cũ đang chạy nếu xác minh đúng tiến trình, sau đó mở bản mới. Nếu Windows chặn thao tác này, đóng SoXeLauncher.exe cũ trong Task Manager rồi chạy lại `Chay-So-Xe.bat`; không cần PowerShell hay setup. Ứng dụng mở bằng trình duyệt mặc định để nhận sẵn các tài khoản Google đã lưu trong trình duyệt và vẫn sử dụng được khi không có Internet.
