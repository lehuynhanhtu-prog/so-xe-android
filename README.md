...
Mã khởi chạy nằm trong `windows-portable/`. Quy trình GitHub Actions biên dịch bộ khởi chạy gọn nhẹ và tạo file `So-Xe-Windows-Portable-1.1.8.zip`. Người dùng chỉ cần giải nén rồi chạy `Chay-So-Xe.bat`; không cần PowerShell hay setup. Ứng dụng mở bằng trình duyệt mặc định để nhận sẵn các tài khoản Google đã lưu trong trình duyệt và vẫn sử dụng được khi không có Internet.
